...
Los objetos creados se deben ligar al contexto de la aplicación, invocando al método **_setAttribute(clave,Objeto)_** del objeto ServletContext. De esta forma, cada vez que desde un servlet de su aplicación, necesite usar el stock de golosinas, deberá invocar al método **_getAttribute()_** del ServletContext para obtener dicho objeto.<br>
**Ayuda**: Para leer del archivo de texto use el método **getResourseAsStream()** de ServletContext que devuelve un objeto InputStream. Luego cree un objeto BufferedReader a partir del objeto InputStream obtenido y así podrá leer de a líneas el archivo de texto.<br>
